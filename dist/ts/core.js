...
exports.Camera = exports.Cartesian = void 0;
// Dependencies
...
class Camera {
    ...
    transform(vec) {
        ...
        // Transformations
        let nVec = math_1.form.translate(vec, math_1.form.negate(this.center));
        nVec = math_1.form.scale(nVec, 1 / this.zoom);
        // Set origin to the top-left
        nVec = [nVec[0], -nVec[1]];
        nVec = math_1.form.translate(nVec, [
    ...
    invTransform(vec) {
        ...
        // Set origin to the top-left
        let nVec = math_1.form.translate(vec, [
            -this.Canvas.cwidth / 2,
            -this.Canvas.cheight / 2
        ]);
        nVec = [nVec[0], -nVec[1]];
        // Transformations
        nVec = math_1.form.scale(nVec, this.zoom);
        nVec = math_1.form.translate(nVec, this.center);
        return nVec;
    }
    getEndpoints() {
...
class Cartesian {
    ...
    axes(x = true, y = true, style = 'black') {
        ...
        let endpoints = this.Camera.getEndpoints();
        if (x)
            this.segment([endpoints[1][0], 0], [endpoints[0][0], 0]);
        if (y)
            this.segment([0, endpoints[2][1]], [0, endpoints[0][1]]);
